...
const Pasti = () => {
  ...
  const { data, error, loading } = useCRUD<Pasti>(
    `/daily_meals/${mensaId}/get_meals_by_id/`
  ); // Note the Pasti[] type
  ...
  const searchMeals = () => {
    ...
    else return data.filter((meal) => meal.type === selectedCategory);
  };
